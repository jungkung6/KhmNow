...
private nonisolated func resolveSignalingUrl(serverIp: String, resourcePath: String, defaultPort: Int = 443) -> String {
    if resourcePath.hasPrefix("rtsps://") || resourcePath.hasPrefix("rtsp://") || resourcePath.hasPrefix("wss://") {
        let withoutScheme: String
        if resourcePath.hasPrefix("rtsps://") {
            withoutScheme = String(resourcePath.dropFirst("rtsps://".count))
        } else if resourcePath.hasPrefix("rtsp://") {
            withoutScheme = String(resourcePath.dropFirst("rtsp://".count))
        } else {
            withoutScheme = String(resourcePath.dropFirst("wss://".count))
        }
        
        let hostAndPort = withoutScheme.components(separatedBy: "/").first ?? ""
        let parts = hostAndPort.components(separatedBy: ":")
        let pathOnly = withoutScheme.contains("/") ? String(withoutScheme.dropFirst(hostAndPort.count)) : "/nvst/"
        let path = pathOnly.hasSuffix("/") ? pathOnly : "\(pathOnly)/"
        
        let urlHost = parts.first ?? ""
        ...
        if urlHost.contains(".") && !urlHost.hasPrefix(".") {
            ...
            if urlHost.hasPrefix(".") {
                if serverIp.hasSuffix(urlHost) {
                    finalHost = serverIp
                } else {
                    finalHost = serverIp + urlHost
                }
            } else {
        ...
        return "wss://\(finalHost):\(urlPort)\(path)"
    }
    
    if resourcePath.hasPrefix("/") {
        let portSuffix = serverIp.contains(":") ? "" : ":\(defaultPort)"
        return "wss://\(serverIp)\(portSuffix)\(resourcePath)"
    }
    let portSuffix = serverIp.contains(":") ? "" : ":\(defaultPort)"
    return "wss://\(serverIp)\(portSuffix)/nvst/"
}
...
final class CloudMatchClient {
    ...
    func createSession(_ input: SessionCreateRequest) async throws -> SessionInfo {
        ...
        if statusCode == 200 {
            ...
            if let payload = try? JSONDecoder().decode(CloudMatchResponse.self, from: data) {
                print("[CloudMatch] Re-using session \(payload.session.sessionId) from HTTP \(statusCode) response")
                return try toSessionInfo(base: base, payload: payload, rawData: data, clientId: clientId, deviceId: deviceId)
            }
        }
    ...
    func stopSession(sessionId: String, token: String, base: String, clientId: String? = nil, deviceId: String? = nil) async throws {
    ...
    func getActiveSessions(token: String, base: String) async throws -> [ActiveSessionInfo] {
        ...
        return (resp.sessions ?? []).filter { $0.status == 1 || $0.status == 2 || $0.status == 3 }.map { entry in
            ...
            let defaultPort = sigConn?.port ?? 443
            let signalingUrl = resolveSignalingUrl(serverIp: serverIp, resourcePath: resourcePath, defaultPort: defaultPort)
            return ActiveSessionInfo(
                sessionId: entry.sessionId,
                status: entry.status,
                appId: appId,
                serverIp: serverIp.isEmpty ? nil : serverIp,
    ...
    private func toSessionInfo(base: String, payload: CloudMatchResponse, rawData: Data, clientId: String, deviceId: String) throws -> SessionInfo {
        if let jsonObject = try? JSONSerialization.jsonObject(with: rawData),
           ...
           let prettyString = String(data: prettyData, encoding: .utf8) {
            print("[CloudMatch] Response Body:\n\(prettyString)")
        } else if let rawString = String(data: rawData, encoding: .utf8) {
            print("[CloudMatch] Response Body (raw): \(rawString)")
        }
        ...
        let resourcePath = sigConn?.resourcePath ?? "/nvst/"
        let defaultPort = (sigConn?.port ?? 0) > 0 ? sigConn!.port : 443
        let signalingUrl = resolveSignalingUrl(serverIp: serverIp, resourcePath: resourcePath, defaultPort: defaultPort)
        ...
        return SessionInfo(
            ...
            signalingUrl: signalingUrl,
            gpuType: s.gpuType,
            queuePosition: s.seatSetupInfo?.queuePosition ?? s.queuePosition,
            seatSetupStep: s.seatSetupInfo?.seatSetupStep ?? s.seatSetupStep,
