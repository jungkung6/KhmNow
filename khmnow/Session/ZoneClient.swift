...
nonisolated struct GFNZone: Identifiable, Equatable {
...
final class ZoneClient {
    ...
    func fetchZones() async throws -> [GFNZone] {
        ...
        return queueData
            .filter { id, _ in id.hasPrefix("NP") && !nukedIds.contains(id) }
            .map { zoneId, entry in
    ...
    private nonisolated struct QueueEntry: Decodable {
        let QueuePosition: Int
        let Region: String
        let eta: Double?
    ...
    private nonisolated struct MappingEntry: Decodable {
        let nuked: Bool?
    }
    ...
    private func fetchQueueData() async throws -> [String: QueueEntry] {
        ...
        var req = URLRequest(url: url)
        req.setValue("KhmNow/1.0 tvOS", forHTTPHeaderField: "User-Agent")
        req.timeoutInterval = 10
        let (data, _) = try await urlSession.data(for: req)
        nonisolated struct Response: Decodable { let data: [String: QueueEntry] }
    ...
    private func fetchMappingData() async throws -> [String: MappingEntry] {
        ...
        req.timeoutInterval = 10
        let (data, _) = try await urlSession.data(for: req)
        nonisolated struct Response: Decodable { let data: [String: MappingEntry] }
        return try JSONDecoder().decode(Response.self, from: data).data
    }
}
