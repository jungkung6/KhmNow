...
class GamesViewModel {
    ...
    init(urlSession: URLSession = .gfnShared) {
        ...
        // Ensure screenMax is at least 60 to prevent PAL 50Hz display clamping.
        let screenMax = max(60, (UIApplication.shared.connectedScenes
    ...
    var availableFps: [Int] {
        // Ensure maxFps is at least 60 to prevent PAL 50Hz display mode from filtering out 60 fps.
        let maxFps = max(60, (UIApplication.shared.connectedScenes
            .compactMap { $0 as? UIWindowScene }
            .first?.screen.maximumFramesPerSecond) ?? 60)
        guard let resos = subscription?.entitledResolutions, !resos.isEmpty else {
    ...
    func game(for session: ActiveSessionInfo) -> GameInfo? {
        guard let appId = session.appId else { return nil }
        if let g = libraryGames.first(where: { $0.variants.contains { $0.appId == appId } }) {
            return g
        }
        if let g = mainGames.first(where: { $0.variants.contains { $0.appId == appId } }) {
            return g
        }
        return nil
    }

    func terminateSession(_ session: ActiveSessionInfo, authManager: AuthManager) async {
        guard let token = try? await authManager.resolveToken() else { return }
        let streamingUrl = authManager.session?.provider.streamingServiceUrl ?? NVIDIAAuth.defaultStreamingUrl
        let base = streamingUrl.hasSuffix("/") ? String(streamingUrl.dropLast()) : streamingUrl
    ...
    func hasPreferredStore(for game: GameInfo) -> Bool {
        preferredStoreIds[game.id] != nil
    }
