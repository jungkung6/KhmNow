...
final class InputSender {
    ...
    // Extended controller mouse state tracking
    private var lastExtendedButtonA: [Int: Bool] = [:]
    private var lastExtendedButtonB: [Int: Bool] = [:]
    ...
    private func applyRemoteMode() {
        ...
        lastExtendedButtonA.removeAll()
        lastExtendedButtonB.removeAll()
    ...
    private func tick() {
        ...
        if remoteMode == .gamepad || remoteMode == .dualsense {
            ...
            // Mouse mode: extended controller and/or Siri Remote can drive the mouse cursor.
            if !isPaused {
                for (idx, controller) in extended.prefix(4).enumerated() {
                    handleExtendedGamepadAsMouse(controller, index: idx)
                }
                if let remote = micro.first {
                    handleMicroGamepad(remote)
                }
            } else {
                overlayHoldTicks.removeAll()
    ...
    private func handleExtendedGamepadAsMouse(_ controller: GCController, index: Int) {
        ...
        if held {
            let ticks = (overlayHoldTicks[index] ?? 0) + 1
            overlayHoldTicks[index] = ticks
            if ticks == Self.overlayLongPressThreshold {
                menuToggleHandler?()
            }
        } else {
            overlayHoldTicks[index] = 0
        }

        // Move pointer using left thumbstick
        let xAxis = pad.leftThumbstick.xAxis.value
        let yAxis = pad.leftThumbstick.yAxis.value
        
        let valX = abs(xAxis) > deadzone ? xAxis : 0
        let valY = abs(yAxis) > deadzone ? yAxis : 0
        
        if valX != 0 || valY != 0 {
            // Analog stick controls pointer velocity. Adjust multiplier for speed.
            let speedMultiplier: Float = 15.0
            let dx = Int16(clamping: Int((valX * speedMultiplier).rounded()))
            let dy = Int16(clamping: Int((-valY * speedMultiplier).rounded()))
            sendMouseMove(dx: dx, dy: dy)
        }

        // Left Click (Button A)
        let aPressed = pad.buttonA.isPressed
        if aPressed != (lastExtendedButtonA[index] ?? false) {
            lastExtendedButtonA[index] = aPressed
            sendMouseButton(down: aPressed, button: 1)
        }

        // Right Click (Button B)
        let bPressed = pad.buttonB.isPressed
        if bPressed != (lastExtendedButtonB[index] ?? false) {
            lastExtendedButtonB[index] = bPressed
            sendMouseButton(down: bPressed, button: 3)
        }
    }
    ...
    private func registerControllerNotifications() {
        let connectObs = NotificationCenter.default.addObserver(
            ...
            guard let self = self else { return }
            let controller = notif.object as? GCController
            Task { @MainActor in
                if let c = controller {
        ...
        let mouseConnectObs = NotificationCenter.default.addObserver(
            ...
            guard let self = self else { return }
            let mouse = notif.object as? GCMouse
            Task { @MainActor in
                if let m = mouse {
                    self.setupMouseHandlers(for: m)
                }
            }
        }
        ...
        // Seed gamepadBitmap and absolute dpad modes for controllers already connected.
    ...
    private func claimControllerInput(_ controller: GCController) {
        ...
        if let micro = controller.microGamepad {
            let buttons: [GCControllerButtonInput?] = [
                micro.buttonA, micro.buttonX,
                micro.dpad.up, micro.dpad.down, micro.dpad.left, micro.dpad.right
            ]
            for btn in buttons.compactMap({ $0 }) {
                btn.preferredSystemGestureState = .disabled
            }
