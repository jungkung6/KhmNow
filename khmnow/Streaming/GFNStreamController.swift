...
final class GFNStreamController: NSObject {
    ...
    private func handleOffer(sdp: String) async {
        ...
        do {
            ...
            let h265SafeSdp = settings.codec == .h265
                ...
                : codecFilteredSdp
            let mangledAnswerSdp = SDPMunger.injectBandwidth(h265SafeSdp, videoKbps: settings.effectiveMaxBitrateKbps)
    ...
    private func buildNvstSdp(iceUfrag: String, icePwd: String, dtlsFingerprint: String) -> String {
        ...
        let maxBitrateKbps     = settings.effectiveMaxBitrateKbps
        let minBitrateKbps     = max(5000, maxBitrateKbps * 35 / 100)
        let initialBitrateKbps = max(minBitrateKbps, maxBitrateKbps * 70 / 100)

        let fecRepairPercent: Int
        let fecRateDropWindow: Int
        let packetSize: Int
        let useOwd: Int
        var extraVqosLines: [String] = []
        ...
        case .lossResilient:
            ...
            extraVqosLines = [
                ...
                "a=vqos.bw.jitterThresholdUs:5000"
            ]
        case .lowLatency:
            fecRepairPercent = 10
            fecRateDropWindow = 8
        ...
        case .custom:
            fecRepairPercent = settings.customFecRepairPercent
            fecRateDropWindow = 10
            packetSize = settings.customPacketSize
            useOwd = settings.customUseOwd ? 1 : 0
            extraVqosLines = [
                "a=vqos.bw.jitterThresholdUs:\(settings.customJitterThresholdUs)"
            ]
        }
        ...
        var lines: [String] = [
            ...
            "m=video 0 RTP/AVP",
            "a=msid:fbc-video-0",
            "a=vqos.fec.rateDropWindow:\(fecRateDropWindow)",
            "a=vqos.fec.repairPercent:\(fecRepairPercent)",
            "a=vqos.drc.enable:0",
            "a=vqos.dfc.enable:0",
            "a=video.enableRtpNack:1",
            "a=video.packetSize:\(packetSize)",
            "a=bwe.useOwdCongestionControl:\(useOwd)",
    ...
    private func attachMicrophone(to pc: LKRTCPeerConnection) async {
        #if os(tvOS)
        // Record permission isn't requested in the same way on tvOS, or is implicitly granted.
        #else
    ...
    private func addRemoteICE(candidate: String, sdpMid: String?, sdpMLineIndex: Int?) {
        ...
        peerConnection?.add(ice) { error in
            if let error = error {
                print("[ICE] Error adding remote candidate: \(error)")
            }
        }
    }
    ...
    // MARK: - Virtual Keyboard Text Input

    func sendText(_ text: String) async {
        guard let sender = inputSender else { return }
        for char in text {
            if let mapping = keysForCharacter(char) {
                if mapping.modifiers & 0x0001 != 0 {
                    sender.sendKeyEvent(down: true, vk: 0x10, scancode: 0x2A, modifiers: 0)
                }
                sender.sendKeyEvent(down: true, vk: mapping.vk, scancode: mapping.scancode, modifiers: mapping.modifiers)
                try? await Task.sleep(for: .milliseconds(15))
                sender.sendKeyEvent(down: false, vk: mapping.vk, scancode: mapping.scancode, modifiers: mapping.modifiers)
                if mapping.modifiers & 0x0001 != 0 {
                    sender.sendKeyEvent(down: false, vk: 0x10, scancode: 0x2A, modifiers: 0)
                }
                try? await Task.sleep(for: .milliseconds(15))
            }
        }
    ...
    func sendBackspace() {
        guard let sender = inputSender else { return }
        sender.sendKeyEvent(down: true, vk: 0x08, scancode: 0x0E, modifiers: 0)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.015) {
            sender.sendKeyEvent(down: false, vk: 0x08, scancode: 0x0E, modifiers: 0)
        }
    }

    func sendEnter() {
        guard let sender = inputSender else { return }
        sender.sendKeyEvent(down: true, vk: 0x0D, scancode: 0x1C, modifiers: 0)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.015) {
            sender.sendKeyEvent(down: false, vk: 0x0D, scancode: 0x1C, modifiers: 0)
        }
    }

    private func keysForCharacter(_ char: Character) -> (vk: UInt16, scancode: UInt16, modifiers: UInt16)? {
        let s = String(char)
        if let asciiVal = char.asciiValue {
            if asciiVal >= 97 && asciiVal <= 122 { // a-z
                let offset = asciiVal - 97
                let vk = UInt16(0x41 + offset)
                let scancodes: [UInt16] = [
                    0x1E, 0x30, 0x2E, 0x20, 0x12, 0x21, 0x22, 0x23, 0x17, 0x24, 0x25, 0x26, 0x32,
                    0x31, 0x18, 0x19, 0x10, 0x13, 0x1F, 0x14, 0x16, 0x2F, 0x11, 0x2D, 0x15, 0x2C
                ]
                return (vk, scancodes[Int(offset)], 0)
            } else if asciiVal >= 65 && asciiVal <= 90 { // A-Z
                let offset = asciiVal - 65
                let vk = UInt16(0x41 + offset)
                let scancodes: [UInt16] = [
                    0x1E, 0x30, 0x2E, 0x20, 0x12, 0x21, 0x22, 0x23, 0x17, 0x24, 0x25, 0x26, 0x32,
                    0x31, 0x18, 0x19, 0x10, 0x13, 0x1F, 0x14, 0x16, 0x2F, 0x11, 0x2D, 0x15, 0x2C
                ]
                return (vk, scancodes[Int(offset)], 0x0001) // Shift
            } else if asciiVal >= 48 && asciiVal <= 57 { // 0-9
                let offset = asciiVal - 48
                let vk = UInt16(0x30 + offset)
                let scancodes: [UInt16] = [
        ...
        switch s {
        case " ": return (0x20, 0x39, 0)
        case "\n", "\r": return (0x0D, 0x1C, 0)
        case "\t": return (0x09, 0x0F, 0)
        case ")": return (0x30, 0x0B, 0x0001)
        case "!": return (0x31, 0x02, 0x0001)
        case "@": return (0x32, 0x03, 0x0001)
        case "#": return (0x33, 0x04, 0x0001)
        case "$": return (0x34, 0x05, 0x0001)
        case "%": return (0x35, 0x06, 0x0001)
        case "^": return (0x36, 0x07, 0x0001)
        case "&": return (0x37, 0x08, 0x0001)
        case "*": return (0x38, 0x09, 0x0001)
        case "(": return (0x39, 0x0A, 0x0001)
        case "-": return (0xBD, 0x0C, 0)
        case "_": return (0xBD, 0x0C, 0x0001)
        case "=": return (0xBB, 0x0D, 0)
        case "+": return (0xBB, 0x0D, 0x0001)
        case "[": return (0xDB, 0x1A, 0)
        case "{": return (0xDB, 0x1A, 0x0001)
        case "]": return (0xDD, 0x1B, 0)
        case "}": return (0xDD, 0x1B, 0x0001)
        case ";": return (0xBA, 0x27, 0)
        case ":": return (0xBA, 0x27, 0x0001)
        case "'": return (0xDE, 0x28, 0)
        case "\"": return (0xDE, 0x28, 0x0001)
        case ",": return (0xBC, 0x33, 0)
        case "<": return (0xBC, 0x33, 0x0001)
        case ".": return (0xBE, 0x34, 0)
        case ">": return (0xBE, 0x34, 0x0001)
        case "/": return (0xBF, 0x35, 0)
        case "?": return (0xBF, 0x35, 0x0001)
        case "\\": return (0xDC, 0x2B, 0)
        case "|": return (0xDC, 0x2B, 0x0001)
        case "`": return (0xC0, 0x29, 0)
        case "~": return (0xC0, 0x29, 0x0001)
        default: return nil
        }
    }
}
...
extension GFNStreamController: LKRTCDataChannelDelegate {
    ...
    nonisolated func dataChannel(_ dataChannel: LKRTCDataChannel, didReceiveMessageWith buffer: LKRTCDataBuffer) {
        ...
        Task { @MainActor [weak self] in
            ...
            self.protocolVersion = finalVersion
            print("[DataChannel] Input ready — starting InputSender (protocol v\(finalVersion))")
            let sender = InputSender(channel: self)
            sender.setProtocolVersion(finalVersion)
